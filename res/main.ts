...
interface TournamentConfig {
  defaultScoreMode?: 'best' | 'sum';
  maxLeaderboardEntries?: number;
  announceResults?: boolean;
  resultAnnouncementChannel?: string;
}
...
interface TournamentSerialized {
  id: string;
  name: string;
  description?: string;
  roomId?: string;
...
function getLeaderboard(tournament: Tournament): Array<{
  rank: number;
  userId: number;
  userName: string;
  score: number;
  ...
  if (tournament.scoreMode === 'best') {
    const bestEntries = new Map<number, LeaderboardEntry>();
...
async function loadData(): Promise<void> {
  ...
  try {
    ...
    const serialized: TournamentSerialized[] = JSON.parse(data);
...
const pluginModule: PluginModule = {
  ...
  async init(api: PluginApi) {
    const cfg = api.readPluginConfig<TournamentConfig>() ?? {};
    ...
    unsubscribers.push(
      api.events.on('room:gameEnd', async ({ room, rankings }) => {
        ...
        for (const tournament of tournaments.values()) {
          if (tournament.state !== 'active') continue;
    ...
    unsubscribers.push(
      api.events.on('room:beforeCreate', ({ roomId, userId }) => {
        for (const tournament of tournaments.values()) {
          if (tournament.roomId !== roomId) continue;
    ...
    unsubscribers.push(
      api.events.on('room:join', ({ room, user }) => {
        for (const tournament of tournaments.values()) {
          if (tournament.roomId !== room.id) continue;
